...
Diagnostics: {
	...
	getYearDiaryHTML: function Diagnostics_getTotalDiaryHTML(node, y, m, d)
	{
		...
		for(var m=1; m<=12; m++)
		{
			...
			ret += '<tr><th onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0}, {1}, 0);">{1}月</th><td>{2}</td><td class="bar100"><div style="width:{4}px;">&nbsp;</div></td><td>{5}</td></tr>'.format(y, m, counts[m], total, (counts[m]/total)*100, str);
		}
		ret += "</table>";
		return  ret;
	},
	getMonthDiaryHTML: function Diagnostics_getTotalDiaryHTML(node, y, m, d)
	{
		...
		var ret = '<table><tr><th colspan="3" class="diary_year" onclick="Skin.Diagnostics.ChangeDiaryRange(null,0,0,0);">{0}</td></tr>'.format(y);
		ret += '<tr><th colspan="3" class="diary_month" onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0},0,0);">{1}月</td></tr>'.format(y, m);
		for(var day=1; day<=days; day++)
		{
			...
			ret += '<tr><th onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0}, {1}, {2});">{2}日</th><td>{3}</td><td class="bar100"><div style="width:{5}px;">&nbsp;</div></td><td>{6}</td></tr>'.format(y, m, day, counts[day], total, (counts[day]/total)*100, str);
		}
		ret += "</table>";
		return  ret;
	},
	...
	{
		return  this.getMonthDiaryHTML(node, y, m, d);	//未実装～
	},
