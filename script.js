...
Diagnostics: {
	...
	RefreshHours: function Diagnostics_RefreshHours(node)
	{
		...
		for(var i=0; i<24; i++)
		{
			...
			ret += '<tr><th>{0}時台</th><td>{1}件</td>{2}</td>'.format(i, c, g);
		}
		ret += '</table>';
		node.innerHTML = ret;
	},
	...
	getYearDiaryHTML: function Diagnostics_getTotalDiaryHTML(node, y, m, d)
	{
		...
		for(var m=1; m<=12; m++)
		{
			...
			ret += '<tr><th onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0}, {1}, 0);">{1}月</th><td>{2}</td>{4}<td>{5}</td></tr>'.format(y, m, counts[m], total, g, str);
		}
		ret += "</table>";
		return  ret;
	},
	getMonthDiaryHTML: function Diagnostics_getTotalDiaryHTML(node, y, m, d)
	{
		...
		ret += '<th colspan="2" class="diary_month" onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0},0,0);">{1}月</td></tr>'.format(y, m);
		for(var day=1; day<=days; day++)
		{
			var str = this.DiaryRangeToHTML(this.getDiaryRangeD(y, m, day))
			var g = this._getBarGraph(counts[day], total);
			ret += '<tr><th onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0}, {1}, {2});">{2}日</th><td>{3}</td>{5}<td>{6}</td></tr>'.format(y, m, day, counts[day], total, g, str);
		}
		ret += "</table>";
		return  ret;
	},
	getDayDiaryHTML: function Diagnostics_getTotalDiaryHTML(node, y, m, d)
	{
		...
		if (dd[y] && dd[y][m] && dd[y][m][d])
		{
			...
			var ret = '<table><tr><th class="diary_year" onclick="Skin.Diagnostics.ChangeDiaryRange(null,0,0,0);">{0}</td>'.format(y);
			ret += '<th class="diary_month" onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0},0,0);">{1}月</td>'.format(y, m);
			ret += '<th class="diary_day" onclick="Skin.Diagnostics.ChangeDiaryRange(null,{0},{1},0);">{2}日</td></tr>'.format(y, m, d);
			for(var i=0; i<24; i++)
			{
				var c = h[i] ? h[i].length : 0;
				var s = this.DiaryRangeToHTML(this.getRange(h[i]));
				var g = this._getBarGraph(c, t);
				ret += '<tr><th>{0}時台</th><td>{1}件</td>{2}</td><td>{3}</td></tr>'.format(i, c, g, s);
			}
			ret += '</table>';
			return ret;
		}
